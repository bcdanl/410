---
title: Lecture 2
subtitle: Omitted Variable Bias; Panel Data Models
format:
  clean-revealjs:
...
# 👥📈 Panel Data Models {background-color="#1c4982"}
...
# 1️⃣ Pooled OLS (POLS) {background-color="#1c4982"}
...
# 2️⃣ Fixed Effects (FE) {background-color="#1c4982"}
...
<!-- ## 🧮 The FE “Within Transformation” (Why It Works) -->

<!-- FE can be estimated by subtracting each country’s time average: -->
...
# 3️⃣ Two-Way Fixed Effects (TWFE) {background-color="#1c4982"}
...
# 4️⃣ Random Effects (RE) {background-color="#1c4982"}
...
## 🧪 FE vs RE: The Hausman Intuition

The logic is:

- FE is consistent even when $Cov(\alpha_i, x_{it}) \neq 0$
- RE is consistent only if $Cov(\alpha_i, x_{it}) = 0$

So we test whether RE is “safe” by checking whether:
- RE and FE estimates are statistically different

That idea is formalized in the **Hausman test**.



# 🧠 Standard Errors in Panel Data  {background-color="#1c4982"}
...
That means residuals may satisfy:

$$
Cov(\epsilon_{it}, \epsilon_{is}) \neq 0 \quad \text{for } t \neq s
$$

If we ignore this, standard errors are often:
❌ too small → false “significance”
...
# 🌿 Environmental Kuznets Curve (EKC): Growth vs. Pollution  {background-color="#1c4982"}
